...
\begin{discussion}
  ...
  (\Cref{theorem:banach_fixed_point}) applies.
\end{discussion}
\begin{definition}
  Let
...
\end{notation}
\begin{proposition}
  \label{proposition:picard_lindelof_preparation}
  Let
    $t_0 \in \R$,
    $n \in \N^+$,
    $x_0 \in \R^n$,
...
\begin{definition}
  ...
  The \textbf{material derivative} on $V$-valued functions is defined by
  \begin{equation}
...
\end{proposition}
\begin{proposition}
  \label{proposition:material_derivative_property}
  Let
    $I$ be an interval,
    $n \in \N^+$,
    $U$ be an open subset of $\R \times \R^n$.
...
\begin{proof}
  ...
  In the base case $k = 0$ both sides equal to $g \circ \tilde{x}$.
  ...
  \begin{equation}
    \begin{split}
      ...
      & \qquad (\text{\Cref{{proposition:material_derivative_property}}}) \\
...
\begin{proof}
  ...
  \begin{equation}
   ...
   \end{equation}
  Let $q \in [0, 1)$ be arbitrary.
  Take (from the proof of \Cref{proposition:picard_lindelof_preparation})
  $\tau, h \in \R^+$ such that the operator $T$ defined on $C^0(I, V)$ in
  \Cref{equation:ordinary_differential_equation/initial_value_problem/operator}
  is a contraction with Lipschitz constant $q$.
  According to the Banach fixed point theorem
  (\Cref{theorem:banach_fixed_point}), since $C^0$ spaces between closed
  subsets of Euclidean spaces are complete, then there exists a unique
  $x \in C^0(I, V)$ that is a fixed point of $T$.
  ...
  From the previous propoition it follows that $u$ is $k$ times differentiable
  and
  \begin{equation}
