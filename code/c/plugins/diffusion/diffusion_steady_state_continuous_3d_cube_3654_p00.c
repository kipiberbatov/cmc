...
Let
  ...
  . G_D = {(x, y, z) in G | x in {0, 3654}}
  . G_N = {(x, y, z) in G | y in {0, 3654} or z in {0, 3654}}
  . g_D = {(0, y, z) |-> 100, (3654, y, z) |-> 0}
  . g_N = 0
...
This problem has exact solution
  . u(x, y, z) = 100 (3654 - x) / 3654
  . q(x, y, z) = (100 / 3654) dy /\ dz
*/
...
{
  return
    ( ((x[1] == 0. || x[1] == 3654.) && (0. <= x[2] && x[2] <= 3654.)) ||
      ((x[2] == 0. || x[2] == 3654.) && (0. <= x[1] && x[1] <= 3654.))
    ) &&
    (0. <= x[0] && x[0] <= 3654.);
}
...
{
  return 100. * (3654. - x[0]) / 3654.;
}

void diffusion_steady_state_continuous_3d_cube_3654_p00_potential(
