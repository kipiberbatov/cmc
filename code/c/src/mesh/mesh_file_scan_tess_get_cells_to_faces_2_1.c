  ...
  int * cf_2_1, FILE * in, int * status, int cn_2, const int * cfn_2_1)
{
  ...
  for (i = 0; i < cn_2; ++i)
  {
    ...
    mesh_file_scan_tess_skip_int_array(in, status, cfn_2_1[i] + 1);
    if (*status)
    {
      cmc_error_message_position_in_code(__FILE__, __LINE__);
    ...
    cfn_2_1_i = int_file_scan(in);
    if (errno)
    {
      cmc_error_message_position_in_code(__FILE__, __LINE__);
      fprintf(stderr, "cannot scan cfn_2_1[%d]\n", i);
      *status = errno;
      return;
    }
    if (cfn_2_1_i != cfn_2_1[i])
    {
      cmc_error_message_position_in_code(__FILE__, __LINE__);
      fprintf(stderr,
        "cfn_2_1[%d] = %d but it should be equal to %d\n",
        i, cfn_2_1_i, cfn_2_1[i]);
      *status = 1;
      return;
    }
    for (j = 0; j < cfn_2_1_i; ++j)
    {
      tmp = int_file_scan(in);
      if (errno)
      {
        cmc_error_message_position_in_code(__FILE__, __LINE__);
        fprintf(stderr, "cannot scan cf_2_1[%d][%d]\n", i, j);
        *status = errno;
        return;
      }
      cf_2_1[index] = abs(tmp) - 1;
      ++index;
    }
