...
struct mesh_3d_kappa_in
{
  ...
  double a;
  unsigned int vpore_size;
  unsigned int fpore_size;
};
...
void mesh_generate_3d_kappa_init_kappa_out(
  struct mesh_3d_kappa_out *out,
  const char *filepath);
void mesh_generate_3d_kappa_free_kappa_out(struct mesh_3d_kappa_out *out);
#endif
/* function declaration */
void mesh_generate_3d_kappa_scan_stcell(
  double* stcell_volumes,int n, const char *m_name);
void mesh_generate_3d_kappa_scan_stface(
  double* stface_data, int n, const char *m_name);
void mesh_generate_3d_kappa_scan_vpore(
  double * vpore_volumes,int n, const char *m_name);
void mesh_generate_3d_kappa_scan_fpore(
  double ** fpore,int n, const char *m_name);
...
void mesh_generate_3d_kappa_get_cf_3_i(int *cf_3_i, const mesh *m, int i,
                                      int p_id);
...
void mesh_generate_3d_kappa_get_coord_0(
  double *vertex_data, const mesh *m,
  int v_id);
void mesh_generate_3d_kappa_get_center_coord_3(
  double *center_coord,
  const mesh *m, int p_id);
void mesh_generate_3d_kappa_get_center_coord_2(
  double *center_coord,
  const mesh *m, int f_id);
void mesh_generate_3d_kappa_get_center_coord_1(
  double *center_coord,
  const mesh *m, int e_id);
double mesh_generate_3d_kappa_get_distance_0(
  const mesh *m, double *coord_1,
  double *coord_2);
double mesh_generate_3d_kappa_get_distance_cell_to_face_3_2(
  const mesh *m,
  int p_id, int f_id);
double mesh_generate_3d_kappa_get_distance_cell_to_edge_3_1(
  const mesh *m,
  int p_id,
  int e_id);
...
void mesh_generate_3d_kappa_assign_thickness_from_vpore(
  ...
  int vpore_size,
  int *cells_ids);
void mesh_generate_3d_kappa_assign_thickness_edge_from_vpore(
  double *thickness_edge,
  const mesh *m,
  int vpore_size,
  int *cells_ids);
void mesh_generate_3d_kappa_calculate_1(
  double *kappa1,
  const mesh *m,
  double *thickness_edge);
void mesh_generate_3d_kappa_calculate_2(
  double *kappa2,
  int *kappa2_n,
  const mesh *m,
  double *thickness,
  int cfn_2_1_total);
void mesh_generate_3d_kappa_calculate_3(
  double *kappa3,
  int *kappa3_n,
  const mesh *m,
  double *stface_data,
  int vpore_size,
  int *cells_ids);
void mesh_generate_3d_kappa_merge_kappa(
  double *kappa,
  const mesh *m,
  double *kappa1,
  double *kappa2,
...
int main(int argc, char **argv)
{
  ...
  if (argc != 11)
  {
    ...
    fprintf(stderr,
    "number of command line arguments should be 10; instead it is %d\n", argc);
    errno = EINVAL;
    goto end;
  }
...
/* init log */
  ...
  if (errno)
  {
    ...
    fprintf(stderr, "error in function %s: %s\n",
      "mesh_generate_3d_kappa_init_kappa_out", strerror(errno));
    goto end;
  }
#endif
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * m->cn[2], "thickness");
    goto end;
  }
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * m->cn[1], "thickness_edge");
    goto free_thickness;
  }
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double)*in->vpore_size, "vpore_volumes");
    goto free_thickness_edge;
  }
  mesh_generate_3d_kappa_scan_vpore(
    vpore_volumes,
    in->vpore_size,
    in->vpore_filepath);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "error in function %s: %s\n",
      "mesh_generate_3d_kappa_scan_vpore", strerror(errno));
    goto free_vpore;
  }
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double*)*2, "fpore");
    goto free_vpore;
  }
  fpore[0] = (double*)malloc(sizeof(double)*in->fpore_size);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double)*in->fpore_size, "fpore[0]");
    goto free_vpore;
  }
  fpore[1] = (double*)malloc(sizeof(double)*in->fpore_size);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double)*in->fpore_size, "fpore[1]");
    goto free_vpore;
  }
  mesh_generate_3d_kappa_scan_fpore(
    fpore,
    in->fpore_size,
    in->fpore_filepath);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "error in function %s: %s\n",
      "mesh_generate_3d_kappa_scan_fpore", strerror(errno));
    goto free_fpore;
  }
  ...
  mesh_generate_3d_kappa_scan_stcell(
    stcell_volumes, m->cn[3], in->stcell_filepath);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "error in function %s: %s\n",
      "mesh_generate_3d_kappa_scan_stcell", strerror(errno));
    goto free_fpore;
  }

  /* 5.Go through all vpore_volumes, current selected denoted as vpore_v */
  int *cells_ids = (int *)malloc(sizeof(int) * in->vpore_size);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(int) * in->vpore_size, "cells_ids");
    goto free_stcell;
  }
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(int) * m->cn[2], "face_ids_used");
    goto free_cells_ids;
  }
  int curr_face_id = 0;
  memset(face_ids_used, 0, m->cn[2]);

  /* 6.Compare vpore volume and 3-cell volume,
       find the closest one for each vpore. */
  mesh_generate_3d_kappa_get_cells_and_faces_from_vpore(
    cells_ids,
    face_ids_used,
    &curr_face_id,
    m,
  ...
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "error in function %s: %s\n",
      "mesh_generate_3d_kappa_assign_thickness_from_vpore", strerror(errno));
    goto free_face_ids_used;
  }

  mesh_generate_3d_kappa_assign_thickness_edge_from_vpore(
    thickness_edge,
    m,
    in->vpore_size,
    cells_ids);
  ...
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "error in function %s: %s\n",
      "mesh_generate_3d_kappa_get_cell_model_volume", strerror(errno));
    goto free_face_ids_used;
  }
  ...
  double *fpore_used = (double *)malloc(sizeof(double) * in->fpore_size);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * in->fpore_size, "fpore_used");
    goto free_face_ids_used;
  }
  double *fpore_volume_used = (double *)malloc(sizeof(double) * in->fpore_size);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * in->fpore_size,
      "fpore_volume_used");
    goto free_fpore_used;
  }

  double *stface_data = (double *)malloc(sizeof(double) * m->cn[2]);
  mesh_generate_3d_kappa_scan_stface(
    stface_data,
    m->cn[2],
    in->stface_filepath);
  if (errno)
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "error in function %s: %s\n",
      "mesh_generate_3d_kappa_scan_stface", strerror(errno));
    goto free_fpore_volume_used;
  }
  int fpore_used_sum;
  /* calculate thickness_edge by vpore */
  /* 8. Select fpore volume by  generating a random number from 0 to 1 and
        using this number to map cdf to volume.*/
  ...
  double_array_file_print_raw_new_line(
    out.thickness_edge_out,
    m->cn[1],
    thickness_edge);
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * kappa1_size, "kappa2");
    goto free_stface;
  }
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * cfn_2_1_total, "kappa2");
    goto free_kappa1;
  }
  memset(kappa2, 0, sizeof(double) * cfn_2_1_total);
  mesh_generate_3d_kappa_calculate_2(
    kappa2,
    &kappa2_n,
    m,
    thickness,
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * cfn_3_2_total, "kappa3");
    goto free_kappa2;
  }
  memset(kappa3, 0, sizeof(double) * cfn_3_2_total);
  mesh_generate_3d_kappa_calculate_3(kappa3, &kappa3_n, m, stface_data,
                                     in->vpore_size, cells_ids);
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * kappa_size, "kappa");
    goto free_kappa3;
  }
  memset(kappa, 0, sizeof(double) * kappa_size);
  mesh_generate_3d_kappa_merge_kappa(
    kappa,
    m,
    kappa1,
    kappa2,
...
void mesh_generate_3d_kappa_init_kappa_out(struct mesh_3d_kappa_out *out,
                                           const char *filepath)
{
  ...
  double * stcell_volumes, int n, const char * m_name)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "cannot open stcell file %s: %s\n", m_name,
      strerror(errno));
    return;
  }
  ...
  double * stface_data, int n, const char * m_name)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "cannot open stface file %s: %s\n", m_name,
      strerror(errno));
    return;
  }
  ...
  for (int i = 0; i < n; i++)
  {
    stface_data[i] = double_file_scan(in);
  }
  ...
  double * vpore_volumes, int n, const char * m_name)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "cannot open vpore file %s: %s\n", m_name, strerror(errno));
    return;
  }
  ...
  double ** fpore, int n, const char * m_name)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    fprintf(stderr, "cannot open fpore file %s: %s\n", m_name, strerror(errno));
    return;
  }

  double *fpore_volumes = fpore[0];
  double *fpore_rates = fpore[1];
...
void mesh_generate_3d_kappa_get_cf_3_i(
  int *cf_3_i,
  const mesh *m,
  int i,
  int p_id)
  ...
  int f_id)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(int) * cfn_2_0, "vertex_ids");
    free(coord);
    return;
  }
  ...
  int e_id)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(int) * cfn_1_0, "vertex_ids");
    free(coord);
    return;
  }
  ...
  int f_id)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * m->dim,
      "polyhedron_center_coord");
    return 0.0;
  }
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * m->dim, "face_center_coord");
  ...
  double distance = mesh_generate_3d_kappa_get_distance_0(
    m,
    polyhedron_center_coord,
    face_center_coord);
  ...
  int e_id)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * m->dim, "edge_center_coord");
    free(polyhedron_center_coord);
    return 0.0;
  }
...
double mesh_generate_3d_kappa_get_cell_model_volume(const mesh *m)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * 3, "vertex_xyz");
    return 0.0;
  }
  ...
  int vpore_size)
{
  ...
  {
    cmc_error_message_position_in_code(__FILE__, __LINE__);
    cmc_error_message_malloc(sizeof(double) * vpore_size, "vpore_diffs");
    return;
  }
  ...
  /* Compare selected vpore_v and all stcell_volumes,
     find out closest stcell_volume, save this stcell_volume's id */
  for (int i = 0; i < vpore_size; i++)
  {
    ...
    {
      cmc_error_message_position_in_code(__FILE__, __LINE__);
      cmc_error_message_malloc(sizeof(int) * cfn_3_2, "cf_3_2");
      free(vpore_diffs);
      return;
    }
...
void get_special_fpore_volume(
  double *fpore_volume,
  double random_fpore_i,
  int fpore_size,
  int scale,
  double **fpore)
{
  for (int j = 0; j < fpore_size; j++)
  {
    ...
    else if (random_fpore_i < fpore[1][j])
    {
      ...
      *fpore_volume =
    ((random_fpore_i - last_rate)
     / (fpore[1][j] - last_rate))
    * (fpore[0][j] - last_volume)
    + last_volume;
      break;
    }
  }
  ...
  unsigned long long * seed)
{
  ...
  *fpore_used_sum = 0;
  ...
  for (; c + b < a;)
  {
    ...
    get_special_fpore_face(
      &select_face_id, face_ids_used,
      curr_face_id, m,
      fpore_volume,
      fpore,
      stface_data,
    ...
    if (select_face_id < m->cn[2])
    {
      ...
      {
        cmc_error_message_position_in_code(__FILE__, __LINE__);
        cmc_error_message_malloc(sizeof(int) * cfn_2_1, "cf_2_1");
        return;
      }
      ...
      {
        if (thickness_edge[cf_2_1[j]] == 0
            || thickness_edge[cf_2_1[j]]
            < thickness[select_face_id])
  ...
  int *cells_ids)
{
  for (int i = 0; i < vpore_size; i++)
  {
    ...
    {
      double value = mesh_generate_3d_kappa_get_distance_cell_to_face_3_2(
        m,
        polyhedron_id,
        cf_3_2[k]);
  ...
  int *cells_ids)
{
  for (int i = 0; i < vpore_size; i++)
  {
    ...
    {
      cmc_error_message_position_in_code(__FILE__, __LINE__);
      cmc_error_message_malloc(sizeof(int) * cfn_3_1, "cf_3_1");
      return;
    }
    ...
    {
      double value = mesh_generate_3d_kappa_get_distance_cell_to_edge_3_1(
        m,
        polyhedron_id,
        cf_3_1[k]);
      if (errno)
  ...
  *kappa3_n = 0;
  ...
  for (int i = 0; i < m->cn[3]; i++)
  {
    ...
    *kappa3_n += cfn_3_2;
    ...
    for (int k = kappa3_index; k < *kappa3_n; k++)
    {
      kappa3[k] = (stface_data[cf_3_2[k - kappa3_index]] / M_PI)
                  / (8 * pow(10, -3));
    }
    kappa3_index += cfn_3_2;
    free(cf_3_2);
